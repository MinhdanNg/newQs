  ...
  <div id="addSubjectContainer">
    <form id="subjectForm">
      ...
      <textarea
        id="teachers"
        name="teachers"
        placeholder="Format: Etternavn, fornavn, epost (linjeskift)"
        v-model="teachers"
      ></textarea>
        ...
        <div id="allSubgroups">
          ...
          <div v-if="tasklist.length > 0">
            ...
            <div v-for="task in tasklist" :key="task">
              <input
                type="checkbox"
                v-model="currentSubgroupChecked"
                :value="task"
              /><label>{{ task }}</label>
            </div>
            ...
            <select v-model="currentSubgroupObligatory">
              <option
                :value="n"
                v-for="n in currentSubgroupChecked.length"
                :key="n"
              >
                {{ n }}
              </option>
  ...
  methods: {
    ...
    addSubgroup() {
      ...
      this.tasklist.splice(
        this.tasklist.indexOf(this.currentSubgroupChecked[0]),
        this.currentSubgroupChecked.length
      );
