<template>
  <div>
    ...
    <form id="userForm">
      ...
      <div>
        <input type="radio" value="student" v-model="role" checked />
        <label>Student</label>
      ...
      <div v-if="role === 'student'">
        ...
        <div class="allSubjects">
          <div v-for="(subject, index) in filteredList" :key="index">
            ...
            />
            <label>{{ subject.subjectCode + " " + subject.subjectName }}</label>
          </div>
        </div>
        <hr />
        <input type="checkbox" v-model="isTeachAss" /><label
          >Læringsassistent</label
        >
        <div v-if="isTeachAss">
