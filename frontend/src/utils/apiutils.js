...
export function addUser(userInfo) {
  return axios.post("http://localhost:8085/api/user/register", userInfo, {
    headers: {
      Authorization: "Bearer " + store.getters.token,
    },
  });
...
export function addSubject(subjectInfo) {
  return axios.post("http://localhost:8085/api/subject/register", subjectInfo, {
    headers: {
      Authorization: "Bearer " + store.getters.token,
    },
  });
...
export function deleteSubject(subjectID) {
  return axios.delete(
    "http://localhost:8085/api/subject/" + subjectID + "/delete",
    {
      headers: {
        Authorization: "Bearer " + store.getters.token,
      },
...
export function archiveSubject(subjectID) {
  return axios.post(
    "http://localhost:8085/api/subject/" + subjectID + "/archive",
    {
      headers: {
        Authorization: "Bearer " + store.getters.token,
      },
...
export function getMyTasks(subjectID) {
  return axios
    .get(
      "http://localhost:8085/api/subject/" +
        subjectID +
        "/get-my-task-overview",
      {
        headers: {
          Authorization: "Bearer " + store.getters.token,
        },
...
export function addToQueue(subjectID, queueInfo) {
  return axios.post(
    "http://localhost:8085/api/subject/" + subjectID + "/queue/join",
    queueInfo,
    {
      headers: {
        Authorization: "Bearer " + store.getters.token,
